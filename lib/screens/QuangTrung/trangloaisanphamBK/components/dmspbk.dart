...
class sanphamdanhmuc extends StatefulWidget {
  const sanphamdanhmuc({
    Key? key,
  }) : super(key: key);

  @override
  State<sanphamdanhmuc> createState() => _sanphamdanhmucState();
}

class _sanphamdanhmucState extends State<sanphamdanhmuc> {

  @override
  void initState() {
    super.initState();
    final sanphams = Provider.of<SanphamProvider>(context, listen: false);
    sanphams.danhmucsanphamBK(context);
  }
          ...
          builder: (context, state, child){
            return Container(
              ...
              child: ListView.builder(
                itemCount: state.danhmucsanphamBKs.length,
                //scrollDirection: Axis.horizontal,
                itemBuilder: (BuildContext context, int index){
                  return dmspshow(
                    hinhanh: state.danhmucsanphamBKs[index].hinhanh,
                    sanphamName: state.danhmucsanphamBKs[index].sanphamName,
                    dongia: state.danhmucsanphamBKs[index].dongia,
                    );
                }
              ),
            );
